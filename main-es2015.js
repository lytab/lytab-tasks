...
let TasksComponent = class TasksComponent {
    ...
    getTasks() {
        this.taskService.getTasks().subscribe(data => { this.tasksFilter = this.tasks = data; });
    }
    deleteTask(task) {
        this.taskService.delete(task.id).subscribe(() => {
            let index = this.tasksFilter.indexOf(task);
            this.tasksFilter.splice(index, 1);
        });
    }
    persistTask() {
        this.taskService.persist(this.myTask).subscribe((data) => {
            this.tasksFilter.push(data);
    ...
    seachTasks() {
        this.tasksFilter = this.tasks.filter((task) => task.label.toLowerCase()
...
let TaskService = class TaskService {
    ...
    update(task) {
        return this.http.put(`${this.url}/${task.id}`, { label: task.label, completed: task.completed });
    }
};
...
module.exports = __webpack_require__(/*! D:\Projet2020\Angular\Angular8\Mohammed Idbrahim\tasks\src\main.ts */"./src/main.ts");
